...
UPDATE_TYPES = {"coupon", "promotion", "flash_sale", "announcement"}
...
def list_feed(user_id: UUID, limit: int, offset: int) -> List[dict]:
    ...
    rows = db_session.exec(
        ...
        .where(StoreFeedPost.store_id.in_(
            select(StoreFollow.store_id).where(StoreFollow.user_id == user_id)
        ))
...
def count_feed(user_id: UUID) -> int:
    return db_session.exec(
        select(func.count()).select_from(StoreFeedPost).where(StoreFeedPost.store_id.in_(
            select(StoreFollow.store_id).where(StoreFollow.user_id == user_id)
        ))
    ).one()
